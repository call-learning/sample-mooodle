 ...
 *
 * define('TEST_SESSION_REDIS_SENTINEL_HOST', '127.0.0.1');
 * define('TEST_SESSION_REDIS_SENTINEL_MASTER_NAME','mymaster');
 *
 * @package    core
...
/**
 * Unit tests for classes/session/redis_sentinel.php.
 *
 * @package    core
 * @copyright  2020 Laurent David <laurent@call-learning.fr>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_session_redis_sentinel_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        if (!defined('TEST_SESSION_REDIS_SENTINEL_HOST') ||
            !defined('TEST_SESSION_REDIS_SENTINEL_MASTER_NAME')) {
            $this->markTestSkipped('Session test server not set. define: TEST_SESSION_REDIS_SENTINEL_HOST'
                . 'and TEST_SESSION_REDIS_SENTINEL_MASTER_NAME');
        }
        ...
        $CFG->session_redis_sentinel_host = TEST_SESSION_REDIS_SENTINEL_HOST;;
        $CFG->session_redis_sentinel_port = defined('TEST_SESSION_REDIS_SENTINEL_PORT') ?
            TEST_SESSION_REDIS_SENTINEL_PORT : 26379;
        $CFG->session_redis_master_name = TEST_SESSION_REDIS_SENTINEL_MASTER_NAME;
        ...
        // We obtain the direct connexion to the Redis Master.
        list($rhost, $rport) = \core\local\redis_helper::get_redis_master_from_sentinel(
            $CFG->session_redis_sentinel_host,
            $CFG->session_redis_sentinel_port,
            $CFG->session_redis_master_name);
        $this->redis = new Redis();
        $this->redis->connect($rhost, $rport); // Redis master
    }
    ...
    public function test_normal_session_read_only() {
        $sess = new \core\session\redis_sentinel();
    ...
    public function test_session_blocks_with_existing_session() {
        ...
        $sessblocked = new \core\session\redis_sentinel();
        $sessblocked->init();
    ...
    public function test_multiple_sessions_do_not_interfere_with_each_other() {
        $sess1 = new \core\session\redis_sentinel();
        $sess1->set_requires_write_lock(true);
        $sess1->init();
        $sess2 = new \core\session\redis_sentinel();
    ...
    public function test_exception_when_connection_attempts_exceeded() {
        ...
        $CFG->session_redis_sentinel_port = 111111;
        $actual = '';
        ...
        $expected = 'Session handler is misconfigured (redis sentinel did not answer:Invalid port)';
